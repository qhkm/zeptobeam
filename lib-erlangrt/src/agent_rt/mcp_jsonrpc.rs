...
impl JsonRpcResponse {
    ...
    pub fn success<T: Serialize>(id: Option<Value>, result: T) -> Result<Self, serde_json::Error> {
        Ok(Self {
            jsonrpc: JSONRPC_VERSION.to_string(),
            id,
...
mod tests {
    ...
    fn test_request_serialization() {
        let request = JsonRpcRequest::new(
            ...
            "initialize",
        ).with_params(serde_json::json!({ "protocolVersion": "2024-11-05" })).unwrap();
    ...
    fn test_error_response() {
        let response = JsonRpcResponse::error(Some(Value::String("1".to_string())), -32601, "Method not found");
        let json = serde_json::to_string(&response).unwrap();
