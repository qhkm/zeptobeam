  ...
  pub fn run_blocking(&self) -> io::Result<()> {
    ...
    for line in stdin.lock().lines() {
      ...
      if is_initialize && response.error.is_none() {
        session_id = self.state.most_recent_session_id();
      }
...
mod tests {
  use super::*;
  use crate::agent_rt::{
    mcp_server::{McpServerState, RuntimeHandle},
  ...
  fn test_jsonrpc_request_parsing() {
    ...
    let request = JsonRpcRequest::new(Some(serde_json::json!(1)), "initialize")
      .with_params(crate::agent_rt::mcp_types::InitializeParams {
        protocol_version: crate::agent_rt::mcp_server::SUPPORTED_PROTOCOL_VERSION
          .to_string(),
        capabilities: None,
        client_info: None,
      })
