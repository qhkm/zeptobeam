...
impl RuntimeHandle {
    ...
    pub fn new(scheduler: Arc<Mutex<AgentScheduler>>, metrics: Arc<RuntimeMetrics>) -> Self {
...
impl McpServerState {
    ...
    pub fn get_available_tools(&self) -> Vec<ToolInfo> {
        vec![
            ...
            ToolInfo::new("get_orchestration_status", get_orchestration_status_schema())
                .unwrap()
...
impl McpProtocolHandler {
    ...
    fn handle_tools_call(
        ...
        let params: ToolsCallParams = match &request.params {
            Some(p) => serde_json::from_value(p.clone())?,
            None => return Err(McpError::InvalidParams("Missing tools/call params".to_string())),
        };

        let result = match params.name.as_str() {
            "spawn_orchestration" => self.handle_spawn_orchestration(params.arguments),
            "get_orchestration_status" => self.handle_get_orchestration_status(params.arguments),
    ...
    fn handle_spawn_orchestration(
        ...
        let args = args.ok_or_else(|| McpError::InvalidParams("Missing arguments".to_string()))?;
        let _goal = args
            .get("goal")
            .and_then(|v| v.as_str())
    ...
    fn handle_get_orchestration_status(
        ...
        let orchestration_id = args
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| McpError::InvalidParams("Missing 'orchestration_id' parameter".to_string()))?;
    ...
    fn handle_send_message(&self, args: Option<serde_json::Value>) -> Result<String, McpError> {
        let args = args.ok_or_else(|| McpError::InvalidParams("Missing arguments".to_string()))?;
        let pid = args
            .get("pid")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse::<u64>().ok())
            .ok_or_else(|| McpError::InvalidParams("Missing or invalid 'pid' parameter".to_string()))?;
    ...
    fn handle_cancel_process(&self, args: Option<serde_json::Value>) -> Result<String, McpError> {
        let args = args.ok_or_else(|| McpError::InvalidParams("Missing arguments".to_string()))?;
        let pid = args
            .get("pid")
            .and_then(|v| v.as_str())
    ...
    fn to_response(&self, id: Option<serde_json::Value>) -> JsonRpcResponse {
        match self {
            ...
            McpError::ToolNotFound(m) => {
                JsonRpcResponse::error(id, error_codes::TOOL_NOT_FOUND, format!("Tool not found: {}", m))
            }
            McpError::Serialization(e) => {
                JsonRpcResponse::error(id, error_codes::INTERNAL_ERROR, format!("Serialization error: {}", e))
            }
            McpError::Runtime(m) => {
